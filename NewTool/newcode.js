...
worker.onmessage = function (message) {
	...
	if ($("#customSwitch1").is(":checked")) audio.play();
...
function exportData(output, maxValueWin = 0, maxValueLost = 0, arrPre=[]) {
	...
	maxWinPre.innerText = arrPre.length;
...
function selectFunction(inputString, arr, typeWinOrLose, typeContinue) {
	// Select by pointer
	let myFunction;
	if (typeWinOrLose === all) myFunction = allFunc;
	if (typeWinOrLose === both) myFunction = bothFunc;
	if (typeWinOrLose === neither) myFunction = neitherFunc;
	if (typeWinOrLose === justWin) myFunction = justWinFunc;
	if (typeWinOrLose === justLose) myFunction = justLoseFunc;
	if (typeWinOrLose === bothNotContinues) myFunction = bothNotContinuesFunc;
	if (typeWinOrLose === justLoseNotContinues) myFunction = justLoseNotContinuesFunc;
	if (typeWinOrLose === justWinNotContinues) myFunction = justWinNotContinuesFunc;

	let myExtraFunction;
	if (typeContinue === smallerThan) myExtraFunction = smallerThanFunc;
	if (typeContinue === largerThan) myExtraFunction = largerThanFunc;
	if (typeContinue === extractNum) myExtraFunction = extractNumFunc;
	if (typeContinue === ignoreThis) myExtraFunction = ignoreThisFunc;
		...
		for (let i = 0; i <= inputString.length - strLength; i++) {
			...
			if (isNone) {
				...
				if (insideL <= outsideL && insideW <= outsideW) {
					if (insideL >= continueTimes || insideW >= continueTimes) arr.pop();
				} else {
					if (insideL > outsideL || insideW > outsideW) if (insideL >= continueTimes || insideW >= continueTimes) arr.pop();
				}
			}
		}
	}
